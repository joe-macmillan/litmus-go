...
import (
	...
	experimentTypes "github.com/litmuschaos/litmus-go/pkg/generic/kubelet-svc-kill-rancher/types"
...
// PrepareKubeletKillRancher contains preparation steps before chaos injection
func PrepareKubeletKillRancher(experimentsDetails *experimentTypes.ExperimentDetails, clients clients.ClientSets, resultDetails *types.ResultDetails, eventsDetails *types.EventDetails, chaosDetails *types.ChaosDetails) error {
	...
	// Creating the helper pod to perform kubelet kill
			...
			Containers: []apiv1.Container{
				{
					...
					Args:      []string{"-c", fmt.Sprintf("cp %[1]s %[2]s && chmod 400 %[2]s && ssh -o \"StrictHostKeyChecking=no\" -o \"UserKnownHostsFile=/dev/null\" -i %[2]s %[3]s@%[4]s %[5]s", privateKeyPath, emptyDirPath, experimentsDetails.SSHUser, experimentsDetails.TargetNodeIP, experimentsDetails.KillCommand)},
